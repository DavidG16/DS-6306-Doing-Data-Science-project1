...
breweries_per_state %>% ggplot() + geom_bar(aes(State, n), fill="steelblue" ,stat = 'identity') +
    labs(x='', y="Number of Breweries", title='Number of Breweries per State') +theme_economist()+
...
missing.values %>% ggplot() + geom_bar(aes(x=key, y=num.missing), fill="steelblue",stat = 'identity') +
   labs(x='Variable', y="Number of missing values", title='Number of missing values') +theme_economist()+
...
impute_data %>% ggplot() + geom_histogram(aes(ABV), fill="steelblue") +
  labs(x='ABV Value', y="count", title='ABV Distribution') + theme_fivethirtyeight()
...
```{r}
impute_data %>% ggplot() + geom_point(aes(x=ABV, y=imputed_IBU), color="steelblue", size = 1.5) + geom_smooth(aes(x=ABV, y=imputed_IBU),method = "lm")+ labs(x='ABV Value', y="IBU Value", title="Relation between IBU and ABV") + theme_economist() + annotate("text",x=0.12, y=135, label="Correlation:0.56", color="darkred", size=4)

c=cor(impute_data$ABV, impute_data$imputed_IBU, method = "spearman")
```
